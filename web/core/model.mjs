...
export const OFFER_STATUSES=['考虑中','已接受','已婉拒','已失效'];
export const CURRENCIES=['CNY','USD','HKD','EUR','GBP','JPY'];
export const KINDS=['applications','events','experiences','offers'];
export const TYPES=['面试','笔试','测评','宣讲会','准备','其他'];
export const emptyState=()=>({version:3,revision:0,applications:[],events:[],experiences:[],offers:[],history:[],undo:[]});
...
// Additive v3 extension: old workspaces keep their revision and records.
export function hydrateState(state){
 if(state.offers===undefined)return {...state,offers:[]};
 if(!Array.isArray(state.offers))throw Error('Offer 数据格式不正确');
 return state;
}
const amount=(value,label,max=1e9)=>{
 if(value===undefined||value===null||value==='')return null;
 if(!['number','string'].includes(typeof value)||!/^\d+(\.\d{1,2})?$/.test(String(value).trim()))throw Error(label+'须为非负数字，最多两位小数');
 const n=Number(value);if(!Number.isFinite(n)||n>max)throw Error(label+'超出可填写范围');return n;
};
export function offerAnnualFixed(offer){return offer.monthlySalary==null||offer.salaryMonths==null?null:Math.round(offer.monthlySalary*offer.salaryMonths*100)/100;}
...
export function normalize(kind,x){
 ...
 if(kind==='applications'){
  ...
  for(const k of ['company','role','job','location','benefits','equity','salaryNotes','notes','receivedDate','deadline','startDate','status','currency'])out[k]=str(x[k]);
  if(!out.company||!out.role)throw Error('公司和职位必填');
  out.status||='考虑中';out.currency||='CNY';
  if(!OFFER_STATUSES.includes(out.status))throw Error('未知 Offer 状态');
  if(!CURRENCIES.includes(out.currency))throw Error('不支持的薪资币种');
  for(const k of ['receivedDate','deadline','startDate'])if(out[k]&&!validDate(out[k]))throw Error('Offer 日期无效');
  out.monthlySalary=amount(x.monthlySalary,'税前月薪');out.salaryMonths=amount(x.salaryMonths,'计薪月数',24);
  if(out.salaryMonths!==null&&out.salaryMonths<1)throw Error('计薪月数至少为 1');
  out.annualBonus=amount(x.annualBonus,'额外年终奖');out.signOn=amount(x.signOn,'签字费');
...
export function normalizeImport(input){
 ...
 for(const kind of KINDS){const list=input[kind]??[];if(input[kind])found=true;if(!Array.isArray(list)||list.length>5000)throw Error('导入记录过多或格式不正确');out[kind]=list.map(x=>normalize(kind,x));const ids=out[kind].map(x=>x.id);if(new Set(ids).size!==ids.length)throw Error('导入文件包含重复 ID');}
 if(!found)throw Error('未找到 applications、events、experiences 或 offers');return out;
}
...
export function previewImport(state,input){
 const data=normalizeImport(input),changes=[],working=structuredClone(hydrateState(state));
...
export function applyCommand(state,command){
 state=hydrateState(state);const next=structuredClone(state);const c=command||{};const before=Object.fromEntries(KINDS.map(k=>[k,state[k]]));
 if(c.type==='undo'){const previous=next.undo.pop();if(!previous)throw Error('没有可以撤销的操作');Object.assign(next,previous);}
 else {
  if(c.type==='upsert'){
   if(!KINDS.includes(c.kind))throw Error('未知记录类型');
   ...
   if(c.kind==='applications')for(const kind of ['events','experiences','offers'])next[kind]=next[kind].map(x=>x.job===c.id?{...x,job:''}:x);
   ...
   for(const kind of ['events','experiences','offers'])if(next[kind].some(x=>x.job&&!next.applications.some(a=>a.id===x.job)))throw Error('导入包含不存在的关联职位');
  }else throw Error('未知操作');
  if(JSON.stringify(before)===JSON.stringify(Object.fromEntries(KINDS.map(k=>[k,next[k]])))&&JSON.stringify(next.history)===JSON.stringify(state.history))return state;
...
export function exportState(s){return {version:3,applications:s.applications,events:s.events,experiences:s.experiences,offers:s.offers??[],history:s.history};}
